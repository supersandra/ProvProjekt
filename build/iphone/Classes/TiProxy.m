...
//Rare exceptions to indicate a bug in the _provprojekt code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_provprojekt = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_provprojekt]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.provprojekt.module"
